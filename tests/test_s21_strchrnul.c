...
START_TEST (empty_string)
{
	...
	c = 'a';
	ck_assert_ptr_eq(s21_strchrnul(str, c), strchrnul(str, c));
	c = '\0';
	ck_assert_ptr_eq(s21_strchrnul(str, c), strchrnul(str, c));
}
END_TEST
...
Suite	*suite_s21_strchrnul(void)
{
	Suite *s = NULL;
	TCase *tc = NULL;

	s = suite_create("suite_s21_strchrnul");
	tc = tcase_create("tcase_s21_strchrnul");
	if (s != NULL && tc != NULL) {
