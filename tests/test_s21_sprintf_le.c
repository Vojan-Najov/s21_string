...
START_TEST(ldbl_max) {
  ...
  if (buf != NULL && buf21 != NULL) {
    n = LDBL_MAX;
    ret = sprintf(buf, "%.100Le\n", n);
    ret21 = s21_sprintf(buf21, "%.100Le\n", n);
    //    if (ret != ret21 || strcmp(buf, buf21)) {
    //      printf("%d:%s%d:%s\n", ret, buf, ret21, buf21);
    //    }
    ck_assert_int_eq(ret, ret21);
...
Suite *suite_s21_sprintf_le(void) {
  ...
  s = suite_create("s21_sprintf Le");
  tc = tcase_create("TcasLe_s21_sprintf_Le_");
